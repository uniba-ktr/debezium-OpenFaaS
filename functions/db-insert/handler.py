...
def handle(req):
    ...
    print ("Inserting into %s values %s" % (table, str(values)))
    entries = insert_into_db(table, values)
...
def insert_into_db(table, values):
    db_url = "http://" + prest_hostname + ":3000/" + default_db + "/public/" + table
    print (db_url)
    r = requests.post(db_url, json.dumps(values))

    if r.status_code != 201:
        sys.exit("Error with insert-into-db, expected: %d, got: %d\n content: %s" % (201, r.status_code, r.content))
